################################################################################
# This script generates 5-set Venn/UpSet visuals from an Excel workbook with
# sheets: PFIZER-BIONTECH, PFIZER-BIONTECH BIVALENT, MODERNA, MODERNA BIVALENT,
# and JANSSEN. It saves ggVennDiagram, VennDetail, and UpSet plots, and writes a
# combined intersection table joined to the source sheets.
################################################################################
...
# This function automatically finds the appropriate color index
...
get_color_by_row <- function(mycol, i) {
  ...
  if (remainder == 0) remainder <- length(mycol)
  return(as.character(mycol[remainder]))
}

## The following functions are from VennDetail and UpSetR packages.
## We modified some of these functions to adjust the colors of the intersection.
...
plot.Venn <- function(x, type = "venn", col = "black", sep = "_",
  ...
  if (type == "vennpie") {
    print(vennpie(
      result, sep = sep, color = piecolor, revcolor = revcolor,
  ...
  if (type == "upset") {
    if ((is.null(mycol)) || (length(x) > length(mycol))) {
      upset(
        fromList(x), nsets = length(x), sets.x.label = sets.x.label,
...
upset <- function(data, nsets = 5, nintersects = 40, sets = NULL, keep.order = F,
                  set.metadata = NULL, intersections = NULL,
                  matrix.color = "gray23", main.bar.color = "gray23",
                  mainbar.y.label = "Intersection Size", mainbar.y.max = NULL,
                  sets.bar.color = "gray23", plot.title = NA, sets.x.label = "Set Size",
                  point.size = 2.2, line.size = 0.7, mb.ratio = c(0.70, 0.30),
                  expression = NULL, att.pos = NULL, att.color = main.bar.color,
                  order.by = c("freq", "degree"), decreasing = c(T, F),
                  show.numbers = "yes", number.angles = 0, number.colors = NULL,
                  ...
                  text.scale = 1, set_size.angles = 0, set_size.show = FALSE,
                  set_size.numbers_size = NULL, set_size.scale_max = NULL,
  ...
  if (is.null(intersections) == F) {
    ...
    All_Freqs <- UpSetR:::specific_intersections(
      data, first.col, last.col, intersections, order.by, group.by, decreasing,
    ...
    All_Freqs <- UpSetR:::Counter(
      New_data, Num_of_set, first.col, Set_names, nintersects, main.bar.color,
  ...
  if (is.null(attribute.plots) == F) {
    for (i in seq_along(attribute.plots$plots)) {
      if (length(attribute.plots$plots[[i]]$x) != 0) att.x[i] <- attribute.plots$plots[[i]]$x else att.x[i] <- NA
      if (length(attribute.plots$plots[[i]]$y) != 0) att.y[i] <- attribute.plots$plots[[i]]$y else att.y[i] <- NA
    }
  }
  ...
  if (is.null(queries) == F) {
    ...
    Matrix_col <- intersects(
      UpSetR:::QuerieInterData, Intersection, New_data, first.col, Num_of_set,
      All_Freqs, expression, Set_names, palette
    )
    Element <- UpSetR:::SeperateQueries(queries, 1, palette)
    EBar_data <- UpSetR:::ElemBarDat(
      Element, New_data, first.col, expression, Set_names, palette, All_Freqs
    )
  } else {
    Matrix_col <- NULL
  }
  
  Matrix_layout <- UpSetR:::Create_layout(
    Matrix_setup, matrix.color, Matrix_col, matrix.dot.alpha, mycol, intersect.mycol.palette.use
  )
  Set_sizes <- UpSetR:::FindSetFreqs(New_data, first.col, Num_of_set, Set_names, keep.order)
  Bar_Q <- NULL
  if (is.null(queries) == F) {
    Bar_Q <- intersects(
      UpSetR:::QuerieInterBar, Intersection, New_data, first.col, Num_of_set,
      All_Freqs, expression, Set_names, palette
    )
  }
  ...
  if ((is.null(queries) == F) & (is.null(att.x) == F)) {
    QInter_att_data <- intersects(
      UpSetR:::QuerieInterAtt, Intersection, New_data, first.col, Num_of_set,
      att.x, att.y, expression, Set_names, palette
    )
    QElem_att_data <- elements(
      UpSetR:::QuerieElemAtt, Element, New_data, first.col, expression, Set_names,
      att.x, att.y, palette
    )
  }
  AllQueryData <- UpSetR:::combineQueriesData(
    QInter_att_data, QElem_att_data, customAttDat, att.x, att.y
  )
  ...
  Main_bar <- suppressMessages(UpSetR:::Make_main_bar(
    All_Freqs, Bar_Q, show.numbers, mb.ratio, customQBar, number.angles, number.colors,
    EBar_data, mainbar.y.label, mainbar.y.max, scale.intersections, text.scale,
    attribute.plots, plot.title
  ))
  Matrix <- UpSetR:::Make_matrix_plot(
    Matrix_layout, Set_sizes, All_Freqs, point.size, line.size,
    text.scale, labels, ShadingData, shade.alpha, intersect.line.color = intersect.line.color
  )
  Sizes <- UpSetR:::Make_size_plot(
    Set_sizes, sets.bar.color, mb.ratio, sets.x.label, scale.sets, text.scale,
    set_size.angles, set_size.show, set_size.scale_max, set_size.numbers_size
  )
...
Make_main_bar <- function(Main_bar_data, Q, show_num, ratios, customQ, number_angles, number.colors,
  ...
  if (is.null(attribute_plots) == FALSE) bottom_margin <- (-1) * 0.45
  ...
  if (is.null(Q) == F) {
    inter_data <- Q
    if (nrow(inter_data) != 0) inter_data <- inter_data[order(inter_data$x), ] else inter_data <- NULL
  } else inter_data <- NULL
  
  if (is.null(ebar) == F) {
    elem_data <- ebar
    if (nrow(elem_data) != 0) elem_data <- elem_data[order(elem_data$x), ] else elem_data <- NULL
  } else elem_data <- NULL
  
  # ten_perc creates appropriate space above highest bar so number doesn't get cut off
  if (is.null(ymax) == T) {
  ...
  if (scale_intersections == "log2") {
    Main_bar_data$freq <- round(log2(Main_bar_data$freq), 2); ymax <- log2(ymax)
  }
  if (scale_intersections == "log10") {
    Main_bar_data$freq <- round(log10(Main_bar_data$freq), 2); ymax <- log10(ymax)
  }
  ...
  if ((show_num == "yes") || (show_num == "Yes")) {
    if (is.null(number.colors)) {
      Main_bar_plot <- (Main_bar_plot + geom_text(
        aes_string(label = "freq"), size = 2.2 * intersection_size_number_scale, vjust = -1,
  ...
  bInterDat <- NULL; pInterDat <- NULL
  bCustomDat <- NULL; pCustomDat <- NULL
  bElemDat <- NULL; pElemDat <- NULL
  
  if (is.null(elem_data) == F) {
    bElemDat <- elem_data[which(elem_data$act == T), ]; bElemDat <- bElemDat[order(bElemDat$x), ]
    pElemDat <- elem_data[which(elem_data$act == F), ]
  }
  if (is.null(inter_data) == F) {
    bInterDat <- inter_data[which(inter_data$act == T), ]; bInterDat <- bInterDat[order(bInterDat$x), ]
    pInterDat <- inter_data[which(inter_data$act == F), ]
  }
  if (length(customQ) != 0) {
    pCustomDat <- customQ[which(customQ$act == F), ]
    bCustomDat <- customQ[which(customQ$act == T), ]; bCustomDat <- bCustomDat[order(bCustomDat$x), ]
  }
  if (length(bInterDat) != 0) {
    Main_bar_plot <- Main_bar_plot + geom_bar(
      data = bInterDat, aes_string(x = "x", y = "freq"),
      fill = bInterDat$color, stat = "identity", position = "identity", width = 0.6
    )
  }
  if (length(bElemDat) != 0) {
    Main_bar_plot <- Main_bar_plot + geom_bar(
      data = bElemDat, aes_string(x = "x", y = "freq"),
      fill = bElemDat$color, stat = "identity", position = "identity", width = 0.6
    )
  }
  if (length(bCustomDat) != 0) {
    Main_bar_plot <- (Main_bar_plot + geom_bar(
      data = bCustomDat, aes_string(x = "x", y = "freq2"),
      fill = bCustomDat$color2, stat = "identity", position = "identity", width = 0.6
    ))
  }
  if (length(pCustomDat) != 0) {
    Main_bar_plot <- (Main_bar_plot + geom_point(
      data = pCustomDat, aes_string(x = "x", y = "freq2"), colour = pCustomDat$color2,
      size = 2, shape = 17, position = position_jitter(width = 0.2, height = 0.2)
    ))
  }
  if (length(pInterDat) != 0) {
    Main_bar_plot <- (Main_bar_plot + geom_point(
      data = pInterDat, aes_string(x = "x", y = "freq"),
  ...
  if (length(pElemDat) != 0) {
    Main_bar_plot <- (Main_bar_plot + geom_point(
      data = pElemDat, aes_string(x = "x", y = "freq"),
      position = position_jitter(width = 0.2, height = 0.2),
...
Make_matrix_plot <- function(Mat_data, Set_size_data, Main_bar_data, point_size, line_size,
                             text_scale, labels, shading_data, shade_alpha,
                             intersect.line.color = "gray83") {
  
  if (length(text_scale) == 1) name_size_scale <- text_scale
  if (length(text_scale) > 1 && length(text_scale) <= 6) name_size_scale <- text_scale[5]
  ...
  Matrix_plot <- (ggplot()
                  ...
                  + xlab(NULL) + ylab("   ")
                  + scale_y_continuous(
                    breaks = c(1:nrow(Set_size_data)),
                    limits = c(0.5, (nrow(Set_size_data) + 0.5)),
                    labels = labels, expand = c(0, 0)
                  )
                  + scale_x_continuous(
                    limits = c(0, (nrow(Main_bar_data) + 1)), expand = c(0, 0)
                  )
                  + geom_rect(
                    data = shading_data, aes_string(xmin = "min", xmax = "max",
                                                    ymin = "y_min", ymax = "y_max"),
                    fill = shading_data$shade_color, alpha = shade_alpha
                  )
                  + geom_point(
                    data = Mat_data, aes_string(x = "x", y = "y"), colour = Mat_data$color,
                    size = point_size, alpha = Mat_data$alpha, shape = 16
                  )
                  + geom_line(
                    data = Mat_data, aes_string(group = "Intersection", x = "x", y = "y"),
...
process_venn_diagrams <- function(file_path, prefix) {
  ...
  ggsave(
    file.path(output_dir, paste0(prefix, "_ggVenn_Diagram.pdf")),
  ...
  five_way_venn <- VennDetail::venndetail(venn_list)
  ...
  upset_plot <- plot(five_way_venn, type = "upset")
  ...
  ##############################################################
  # for unknown reason, PDF files are not properly created with upset plot
  upset_plot_updated <- plot(five_way_venn, type = "upset", mycol = custom_colors)
  ...
  # for unknown reason, PDF files are not properly created with upset plot
  upset_plot_updated <- plot(
    five_way_venn, type = "upset", mycol = custom_colors,
  ...
  ###################################################
  ##### Save detailed combs from VennDetail object
  ###################################################
  venn_detail <- result(five_way_venn)
